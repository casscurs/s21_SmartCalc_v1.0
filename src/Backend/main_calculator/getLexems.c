...
int getLexemsArr(char* initial, StackNode* lexeme) {
  char str[256] = {0};
  int unar = 0;
...
void numbToLexeme(StackNode* lexeme, int* lexCount, char* buf, int* bufCount,
                  int* i, int* unar) {
...
void signToLexeme(StackNode* lexeme, char* str, int* lexCount, int* i,
                  int* unar, int* checker) {
  if (str[*i] == 'x') {
    if (*unar == 1) {
...
void fillFunc(StackNode* lexeme, int* lexCount, int* step, int stepVal,
              int what, int priority) {
