...
MainWindow::MainWindow(QWidget* parent)
  ...
  connect(ui->pushButton_With, SIGNAL(clicked()), this, SLOT(pushToStackWith()));
}
...
void MainWindow::on_pushButton_res_clicked() {
  ...
  if (calc(charArray, charArrayX, &res)) {
    ui->lineEdit_input->setText(QString::number(res));
  } else
...
void MainWindow::on_pushButton_calc_clicked() {
  ...
  if (ui->radioButton_anny->isChecked()) {
    int type = ANNUITY;
    if (credit(charArraySum, charArrayPeriod, charArrayPercent, type, &res)) {
      ui->plainTextEdit_output->clear();
      ui->plainTextEdit_output->appendPlainText(
          "Ежемесячный платеж: " + QString::number(res.monthly, 'f', 2));
      ui->plainTextEdit_output->appendPlainText(
          "Переплата: " + QString::number(res.overpay, 'f', 2));
      ui->plainTextEdit_output->appendPlainText(
          "Общая сумма: " + QString::number(res.total, 'f', 2));
    } else
  ...
  if (ui->radioButton_diff->isChecked()) {
    ...
    if (credit(charArraySum, charArrayPeriod, charArrayPercent, type, &res)) {
      ui->plainTextEdit_output->clear();
      double period = 0;
      readX(charArrayPeriod, &period);
      for (int i = 0; i < (int)period; ++i) {
        ui->plainTextEdit_output->appendPlainText(
            "Месяц " + QString::number(i + 1) + ": " +
            QString::number(res.valArr[i], 'f', 2));
      }
      ui->plainTextEdit_output->appendPlainText(
          "Переплата: " + QString::number(res.overpay, 'f', 2));
      ui->plainTextEdit_output->appendPlainText(
...
void MainWindow::on_pushButton_depCalc_clicked() {
  ...
  if (ui->radioButton_yearly->isChecked()) str.regularity = YEARLY;
  ...
  if (deposit(str, &add, &with, &depOut)) {
    ui->plainTextEdit_dep->clear();
    ui->plainTextEdit_dep->appendPlainText(
        "Начисленные проценты: " +
        QString::number(depOut.percentAward, 'f', 2));
    ui->plainTextEdit_dep->appendPlainText(
        "Сумма налога: " + QString::number(depOut.taxAmount, 'f', 2));
    ui->plainTextEdit_dep->appendPlainText(
        "Сумма на вкладе к концу срока: " +
        QString::number(depOut.totalAward, 'f', 2));
  } else
...
void MainWindow::pushToStackAdd() {
  ...
  if (!readX(charArrayAddSum, &addsum) || !readX(charArrayAddMonth, &addmonth)||
...
void MainWindow::pushToStackWith() {
  ...
  if (!readX(charArrayWithSum, &withsum) || !readX(charArrayWithMonth, &withmonth) ||
