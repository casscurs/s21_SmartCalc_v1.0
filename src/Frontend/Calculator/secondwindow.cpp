...
void SecondWindow::on_pushButton_plot_clicked() {
  ...
  if (!readX(charArrayXmin, &Xmin)) answer = 0;
  if (!readX(charArrayXmax, &Xmax)) answer = 0;
  if (!readX(charArrayXrange, &Xrange)) answer = 0;
  if (!readX(charArrayYrange, &Yrange)) answer = 0;
  if (!getLexemsArr(charArrayFunc, lexeme)) answer = 0;
